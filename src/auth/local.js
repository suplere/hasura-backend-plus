...
router.post('/login', async (req, res, next) => {
  ...
  const { username, password, appId } = value;

  let query = `
  query ($username: String!, $appId: uuid) {
    user_accounts: ${schema_name}user_accounts(where: {username: {_eq: $username}}) {
      password
      user {
        id
        active
  ...
  try {
    ...
    return next(Boom.unauthorized("Unable to find 'user' or 'appId'"));
  }

  if (hasura_data[`user_accounts`].length === 0) {
    // console.error("No user with this 'username'");
    return next(Boom.unauthorized("Invalid 'username' or 'password' or 'appId'"));
  }
